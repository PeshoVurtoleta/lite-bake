...
- Torture t8 goes live: seven cross-package pins against the pinned published `@zakkster/lite-bake-stream@1.6.0` -- (a) F64 lane parity through `Reader.fromBytes`, (b) U32 index-vs-interned-string semantics, (c) the lane-code table divergence, (d) the executable wrong-file honesty pin, (e) the src-export vs llms.txt vs d.ts drift guard, (f) the harness `BYTES` table vs src drift guard, (g) the docs-presence pins. Deterministic (fixed corpus, no PRNG); ignores `BAKE_TORTURE_BREAK`; runs outside every gc-profiler window. Pure helpers exported for t9.
- t9 Controls 17-23: one control per t8 pure helper, each with teeth (a doctored input flagged) and a non-vacuity twin (the true-shaped input clean).
- `decisions/0006-wire-format.md`: Option A (delegate) sharpened -- LBK1 is the suite's only wire format, the raw `Reader.fromBytes` lane stays as the exact-layout seam with its two hazards documented, plus five rejected alternatives and the measured XP evidence at sibling 1.6.0.
- The README `## Ecosystem` section and the llms.txt ecosystem stanza + a "how do I persist or ship baked data" agent-mapping bullet.
- devDependency `@zakkster/lite-bake-stream` pinned exactly `1.6.0` (t8 only; never ships in the pack).

### Changed

- README FAQ disk answer: the raw `Reader.fromBytes` lane's hazards are now stated in bold (content-blind by design -- SHAPE is verified, CONTENT never; native-endian) with the LBK1 pointer; the `serialize()` / `deserialize()` roadmap promise is removed (decided against -- 0006).
- README "Native endianness is used throughout" no longer conflates in-process round-trips with cross-machine ones: the baked bytes are **not portable across endianness** (marked in bold) and carry no byte-order marker; LBK1 is the portable, little-endian-specified container.
- README Roadmap rewritten to decided/parked truth: columnar (SoA) parked, string tables delegated to LBK1 U32 lanes, `serialize()`/`deserialize()` resolved by 0006, matrix/normalized-int attributes parked.

### Fixed

- **BK-05** (no-magic half): resolved as a stated, t8-demonstrated contract -- the raw lane detects SHAPE, never CONTENT; the detecting container (LBK1: magic at both ends, strict decode, optional CRC-32C) is the named in-suite path one devDep away.
- **BK-14:** the endianness constraint is documented truth with the portable path named (LBK1 is little-endian-specified); the old "works on both LE and BE" round-trip line is corrected.
- **BK-26:** the ecosystem relationship is stated on this side exactly as measured (the sibling's side is restated in its own BS-29 pass); the lane-code and U32-semantic divergences are pinned by t8 so any future convergence is a deliberate diff, not drift.
